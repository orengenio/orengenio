...
type FooterLink = InternalLink | AffiliateLink;
    ...
    <footer className=\"og-footer bg-white text-black\" aria-label=\"OrenGen Worldwide Footer\">
      ...
      <div className=\"max-w-[1280px] mx-auto px-6\">
        <div className=\"og-brand-section flex flex-col items-center py-16\">
          ...
          <div className=\"flex gap-4 mt-12\">
            {socialLinks.map((social) => (
              <a key={social.id} href={social.url} target=\"_blank\" rel=\"noopener noreferrer\" className=\"w-11 h-11 flex items-center justify-center rounded-xl border border-black/10 hover:border-[#CC5500] hover:bg-[#CC5500]/5 transition-all\">
                <img src={`https://cdn.simpleicons.org/${social.id}/000000`} className=\"w-5 h-5\" alt={`${social.id} profile`} />
              </a>
            ))}
          </div>
        </div>
            ...
            <div key={col.title} className=\"og-col\">
              ...
              <ul className=\"flex flex-col gap-4\">
                {col.links.map((link) => {
                  ...
                  const commonClass = \"text-[13px] font-bold text-black hover:text-[#CC5500] transition-colors\";
                  return (
                    <li key={link.name}>
                      {isExternal ? (
                        <a href={link.href} target=\"_blank\" rel=\"noopener noreferrer\" className={commonClass}>{link.name}</a>
                      ) : (
                        <Link href={link.href} className={commonClass}>{link.name}</Link>
                      )}
                    </li>
                  );
                })}
        ...
        <div className=\"og-bottom-bar py-12 flex flex-col md:flex-row items-center justify-between gap-10 border-t border-black/5\">
          <div className=\"text-[11px] font-bold uppercase tracking-wider text-black/50\">© {currentYear} OrenGen Worldwide LLC. All rights reserved.</div>
